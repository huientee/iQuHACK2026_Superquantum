"""
Challenge 7: something something matrix
"""
...
def main():
    ...
    # complete to a unitary
    Q, _ = np.linalg.qr(U + 1e-12*np.random.randn(4,4))
    U = Q

    # Create UnitaryGate
    unitary_gate = UnitaryGate(U, label="exp(i H theta)")
    ...
    with open("../circuits/challenge_7.qasm", "w") as f:
        f.write(qasm_baseline)
    print("  Saved: challenge_7.qasm")
    ...
    with open("../circuits/challenge_7_synth.qasm", "w") as f:
        f.write(qasm_optimized)
    print("Saved: challenge_7_synth.qasm")
